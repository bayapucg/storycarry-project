...
<script type="text/javascript">
	...
	function writerfollow(writer_id, writer_name) { // follow button 
        ...
        $.ajax({
    		type :'POST',
    		url :'<?php echo base_url(); ?>welcome/follow',
...
$( document ).ready(function() {
    ...
    /*$( document ).ready(function() {
    	...
    	$("input[name='writerstar']").click(function(){
    	    $('#snackbar').text('You cannot rate your own story.').addClass('show');
    		setTimeout(function(){ $('#snackbar').removeClass('show'); }, 3000);
    	});
    }); */
</script>
...
<script>
    ...
    function nanoviewsadd(sid){
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url();?>welcome/nanoviewsadd",
            data: {'sid':sid},
            success: function(data) {
    			if(data == 1) {
    ...
    function nanolike(storyid){ // Nano story likes
        ...
        $.ajax({
    		type :'POST',
    		url :'<?php echo base_url(); ?>welcome/nanolike/'+storyid,
    		dataType :"json",
    		success:function(data){
    		    if(data == 1){
...
<script>
    ...
    function yesongoingcomplet(sid){
        $.ajax({
    		type :'POST',
    		url :'<?php echo base_url(); ?>welcome/ongoingcomplet',
    		data: {'sid':sid},
    		success:function(data){
    		    if(data == 1){
