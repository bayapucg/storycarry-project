    					...
    					<div class="box-comment">
    						...
    						<div style="margin:4px 0 6px 2px" class="comment-text">
    						    <?php if(strlen($comment->comment) > 200){ ?>
    						        <span class="more <?php echo $comment->id;?>"><?php echo substr($comment->comment, 0, 200); ?>
    						            ...
    						            <u onclick="showhide(<?php echo $comment->id;?>)" class="moreless<?php echo $comment->id;?>" style="cursor: pointer;color:red;">show-more</u></span>
    						    <?php } else{ ?>
...
<?php } } if(isset($toploadcommustories) && count($toploadcommustories) > 0){ foreach ($toploadcommustories as $topkey) { ?>
	<div class="box box-widget" style="background:#fff; border:1px solid #dddddd; margin-bottom:10px;">
	    <div class="box-header with-border">
			<div class="user-block">
				...
				<span class="username"><a href="<?php echo base_url().$topkey->profile_name; ?>"><?php echo $topkey->name;?></a>
		...
		<?php if(($topkey->type=='url')) { ?>
			<div class="box-body">
				...
				<?php } else if($topkey->urldescription){ ?>
					...
					<div class="media border-box" style="margin-top:2px;">
						<div class="media-left">
							<a href="<?php echo $topkey->titleurl;?>" target="_blank">
								...
								<?php } else { ?>
								    <img src="<?php echo base_url();?>assets/default/feed_comm.jpeg" class="media-object img-v" alt="<?php echo ucfirst($topkey->title);?>">
								<?php } ?>
							</a>
						</div>
						<div class="media-body" style="padding-top:15px;">
							...
							<?php if(isset($topkey->typeoftype) && ($topkey->typeoftype == 'quotes')){ ?>
							    <p class="sizep text-justify edittext<?php echo $topkey->id;?>" style="margin:0px;"><?php echo $topkey->story;?></p>
							<?php } else if(strpos($topkey->story, '<em> Written by <a href=')) { ?>
							    <p class="edittext<?php echo $topkey->id;?>" style="margin:0px;"><?php echo $topkey->story;?></p>
							<?php } else{ ?>
							    <?php if(isset($topkey->story) && (strlen($topkey->story) > 200)){ ?>
								    <p class="media-heading text-justify edittext<?php echo $topkey->id;?>" style="margin:0px;"><?php echo mb_substr($topkey->story, 0, 200); ?>
						                <span class="showhidedesc<?php echo $topkey->id;?>" style="display:none;"><?php echo mb_substr($topkey->story,200); ?></span>
						                <span class="smorelessdots<?php echo $topkey->id;?>">...</span>
						                <u onclick="showhidedesc(<?php echo $topkey->id;?>)" class="smoreless<?php echo $topkey->id;?>" style="cursor: pointer;color:red;">show-more</u></p>
								<?php } else{ ?>
								    <p class="media-heading text-justify edittext<?php echo $topkey->id;?>" style="margin:0px;"><?php echo $topkey->story;?></p>
								<?php } ?>
							<?php } ?>
						</div>
					</div>
				<?php } else { ?>
					...
					<?php } else if(strpos($topkey->story, '<em> Written by <a href=')) { ?>
					    <p class="edittext<?php echo $topkey->id;?>" style="margin:0px;"><?php echo $topkey->story;?></p>
					<?php } else{ ?>
					    <?php if(isset($topkey->story) && !empty($topkey->story)){ ?> 
					    <?php if (strlen($topkey->story) > 200){ ?>
						    <p class="text-justify edittext<?php echo $topkey->id;?>" style="margin:0px;"><?php echo mb_substr($topkey->story, 0, 200); ?>
				                <span class="showhidedesc<?php echo $topkey->id;?>" style="display:none;"><?php echo mb_substr($topkey->story,200); ?></span>
				                <span class="smorelessdots<?php echo $topkey->id;?>">...</span>
				                <u onclick="showhidedesc(<?php echo $topkey->id;?>)" class="smoreless<?php echo $topkey->id;?>" style="cursor: pointer;color:red;">show-more</u></p>
						<?php } else{ ?>
						    <p class="text-justify edittext<?php echo $topkey->id;?>" style="margin:0px;"><?php echo $topkey->story;?></p>
						<?php } } ?>
					<?php } ?>
				<?php } ?>
			</div>
	    <?php } else { ?>
			<div class="box-body">
			    <?php if(isset($topkey->urldescription) && !empty($topkey->urldescription)){
			        if(strlen($topkey->urldescription) > 200){ ?>
				    <div class="user-block mb-10"><?php echo mb_substr($topkey->urldescription, 0, 200); ?>
		                <span class="showhidedesc<?php echo $topkey->id;?>" style="display:none;"><?php echo mb_substr($topkey->urldescription,200); ?></span>
		                <span class="smorelessdots<?php echo $topkey->id;?>">...</span>
		                <u onclick="showhidedesc(<?php echo $topkey->id;?>)" class="smoreless<?php echo $topkey->id;?>" style="cursor: pointer;color:red;">show-more</u></div>
				...
				<?php } else{ ?>
				    <?php if(isset($topkey->story) && !empty($topkey->story)){ ?> 
				    <?php if (strlen($topkey->story) > 200){ ?>
					    <p class="text-justify edittext<?php echo $topkey->id;?>" style="margin:0px;line-height: 1.85em;    font-size: 1.2em;"><?php echo mb_substr($topkey->story, 0, 200); ?>
			                <span class="showhidedesc<?php echo $topkey->id;?>" style="display:none;"><?php echo mb_substr($topkey->story,200); ?></span>
			                <span class="smorelessdots<?php echo $topkey->id;?>">...</span>
			                <u onclick="showhidedesc(<?php echo $topkey->id;?>)" class="smoreless<?php echo $topkey->id;?>" style="cursor: pointer;color:red;">show-more</u></p>
					<?php } else{ ?>
					    <p class="text-justify edittext<?php echo $topkey->id;?>" style="margin:0px;line-height: 1.85em;font-size: 1.2em;"><?php echo $topkey->story;?></p>
					<?php } } ?>
				<?php } ?>
			</div>
		<?php } ?>
		<div class="box-body">
			...
			<?php } ?>
			<span style="padding-left:5px;" onclick="commusocialshare(<?php echo $topkey->id;?>, '<?php echo preg_replace('/\s+/', '-', $communitie_about); ?>');">
			    <a data-toggle="modal" data-target="#soc" style="color:#333;cursor:pointer;"><i class="fa fa-share"></i> Share</a>
			</span>
		...
		<!-- /.box-footer -->        					
        <div class="box-comments box-footer tajaxcomment<?php echo $topkey->id;?>" style="background: #fff;padding-bottom:0px;border:0px;"> </div>
