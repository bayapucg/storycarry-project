...
<body>
    ...
    <section class="searchv">
        <div class="bgv">
            <!--<div class="searchvv">
                <a href="<?php echo base_url();?>index.php/welcome/ssearch?searchs=a" style="color:#fff;" required></a>
            </div>-->
            <center> 
                <div class="searchvv" id="search">
                    <form method="GET" id="searchpost" action="<?php echo base_url().$this->uri->segment(1);?>/ssearch">
    ...
    <?php if(isset($blogs) && ($blogs->num_rows() > 0)) { ?>
        <div class="main-div" id="loadmoreall">
            <?php foreach($blogs->result() as $blog) { ?>
                <div class="inner-div">
                    <a href="<?php echo base_url().$this->uri->segment(1);?>/blog/<?php echo preg_replace('/\s+/', '-', $blog->caption).'-'.$blog->id;?>">
    ...
    $(document).ready(function() {
        $("#searchitems").autocomplete({
            source: function( request, response ) {
                $.ajax({
                    url: "<?php echo base_url().$this->uri->segment(1);?>/blogsearch",
                    type: 'post',
                    dataType: "json",
...
<script>
    $(document).ready(function(){
        ...
        function load_country_data(limit, start) {
            $.ajax({
                url:'<?php echo base_url().$this->uri->segment(1);?>/blogloadmore',
                method:"POST",
