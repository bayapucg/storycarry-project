    	...
    	<div class=""><!-- container-fluid-->
            <div class="main-container">
        					    ...
        					    <?php if(isset($comm_joines) && ($comm_joines->num_rows() >0)) { ?>
        					        ...
        					        <center>
										<?php $imgcount = 1; foreach($comm_joines->result() as $comm_joinie) {
											...
											<a href="<?php echo base_url().$this->uri->segment(1).'/'.$comm_joinie->profile_name;?>"><span>
            		    	...
            		    	<div class="sidebarvj" style="box-shadow: 0 1px 3px 0 rgba(0,0,0,.2), 0 1px 1px 0 rgba(0,0,0,.14), 0 2px 1px -1px rgba(0,0,0,.12);">
                				<div class="header">
                					<p><b>OTHER COMMUNITIES <span class="pull-right">
                					    <a href="<?php echo base_url($this->uri->segment(1).'/communities');?>">More</a></span>
                				    </b></p>
                				</div><br>
            				...
            				<div class="tab-content">
            					...
            					<div class="tab-pane active" id="post">
            						...
            						<div class="box-footer">
            										...
            										<form id="posturlform">
            											...
            											<div class="input-group" style="width:100%;">
            											    ...
            											    <span class="input-group-btn">
            												    <button type="submit" class="btn btn-success btn-flat posturlformspinner">POST</button>
            												</span>
            											</div>
            										</form>
            									</div>
            											...
            											<form id="postquoteform">
            												<div class="img-push">
            													...
            													<div class="input-group-btn pull-right" style="padding-right: 51px; margin-top:10px;">
            														<button type="submit" class="btn btn-primary postquoteformspinner">Post</button>
            													</div>
            												</div>
            											</form>
            											<br><br>
            										...
            										<div class="box-footer" style="border-top:none;">
            											<form id="posttextform">
            												<div class="img-push">
            													...
            													<div class="input-group-btn pull-right" style="padding-right: 51px; margin-top:10px;">
            														<button type="submit" class="btn btn-primary posttextformspinner">Post</button>
            													</div>
            												</div>
            											</form>
            											<br><br>
            							...
            							<?php if($sesslang != 'en'){ ?>
        								<div style="margin-top: -30px;position: relative; border-collapse: separate;" class="desktopedit hidden-xs hidecommswitch">
            								...
            								<div class="box box-widget" style="border:1px solid #dddddd;margin-bottom:10px;display:none;" id="previewdiv">
            									<div class="box-header with-borders">
            										<div class="user-block">
            											...
            											<span class="description"><!--<?php echo date('Y-m-d'); ?> -->Preview</span>
            										</div>
            									</div>
            									...
            									<div class="box box-widget" style="background:#fff; border:1px solid #dddddd; margin-bottom:10px;" id="delcomm<?php echo $key->id;?>">        
    												<div class="box-header with-borders">
            											<div class="user-block">
            												...
            												<span class="username">
            												    <a href="<?php echo base_url().$this->uri->segment(1).'/'.$key->profile_name;?>">
            												        <p class="namers"><?php echo $key->name;?></p></a> 
        		...
        		<div class="side3">
        			<div class="sidebarvj" style="box-shadow: 0 1px 3px 0 rgba(0,0,0,.2), 0 1px 1px 0 rgba(0,0,0,.14), 0 2px 1px -1px rgba(0,0,0,.12);">
        				<div class="header">
        					<p><b>OTHER COMMUNITIES <span class="pull-right"><a href="<?php echo base_url($this->uri->segment(1).'/communities');?>">More</a></span>
        				    </b></p>
        				</div><br>
...
<script>
    function tab_about(comm_id){ // tab about
        $.ajax({
    		type :'POST',
    		url :'<?php echo base_url().$this->uri->segment(1);?>/community_about',
    ...
    function tab_tpost(comm_id){ // tab top posts
        ...
        $.ajax({
    		type :'POST',
    		url :'<?php echo base_url().$this->uri->segment(1); ?>/community_tpost',
    		data :{'comm_id':comm_id ,'communitie_about': communitie_about},
    		success:function(data){
    		    $('#tpost').html(data);
    		}
        });
    ...
    $('#previewurl, #urldescription').keyup(function() { // post links preview
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url().$this->uri->segment(1);?>/testing",
			...
			success: function(data) {
				if(data == 'wrongurl'){
                    ...
                    if(posturlimage){   }else{ posturlimage = '<?php echo base_url();?>assets/default/feed_comm.jpeg'; }
					$('#previewdiv').css('display','block');
					...
					'<h4 class="media-heading"><b>'+data.title+'</b></h4></a> <p class="media-heading edittext">'+data.story+'</p> </div> </div>');
				} else{
					$('#previewdiv').css('display','block');
	...
	$( "form#posturlform" ).submit(function( event ) { //post url form
        ...
        $('.posturlformspinner').html('<img src="<?php echo base_url();?>/assets/landing/svg/spinner.svg" class="spinner">');
        $('.text-danger').html('');
        $.post("<?php echo base_url().$this->uri->segment(1);?>/communities_story",$("form#posturlform").serialize(),function(resultdata) {
    ...
    $( "form#postquoteform" ).submit(function( event ) { //post quotes form
        event.preventDefault();
        $('.postquoteformspinner').html('<img src="<?php echo base_url();?>/assets/landing/svg/spinner.svg" class="spinner">');
        $('.text-danger').html('');
        $.post("<?php echo base_url().$this->uri->segment(1);?>/communities_story",$("form#postquoteform").serialize(),function(resultdata) {
            if(resultdata == 'notvalid'){
                $('.notvalid').html('Please Enter URL');
            } else if(resultdata == 'wrongurl'){ 
                $('.notvalid').html('Please Enter a Valid URL');
    ...
    $( "form#posttextform" ).submit(function( event ) { //post text form
        event.preventDefault();
        $('.posttextformspinner').html('<img src="<?php echo base_url();?>/assets/landing/svg/spinner.svg" class="spinner">');
        $('.text-danger').html('');
        $.post("<?php echo base_url().$this->uri->segment(1);?>/communities_story",$("form#posttextform").serialize(),function(resultdata) {
            if(resultdata == 'notvalid'){
                $('.notvalid').html('Please Enter URL');
            } else if(resultdata == 'wrongurl'){ 
                $('.notvalid').html('Please Enter a Valid URL');
    ...
    function commujoin(comm_id, gener) { // community join button 
        $.ajax({
    		type :'POST',
    		url :'<?php echo base_url().$this->uri->segment(1); ?>/communities_join',
...
<script type="text/javascript">
	...
	function commloadsubcomment(storyid, comment_id, subcommentid ){
        ...
        $.ajax({
            type:'POST',
            url:"<?php echo base_url().$this->uri->segment(1);?>/commloadsubcomments",
	...
	function editcommcomment(commentid){
        $.ajax({
            type:'POST',
            url:"<?php echo base_url().$this->uri->segment(1);?>/editcommcomment",
    ...
    function updatecommcomment(commentid){
     ...
     function deletecommcomment(commentid){
        ...
        $('#confirmdelpopup').modal().one('click', '#delconfirmed', function (e) {
            $.ajax({
                type:'POST',
                url:"<?php echo base_url().$this->uri->segment(1);?>/deletecommcomment",
                data:{'commentid':commentid},
                success:function(response){
                    if(response){
    ...
    function reportcommentdiv(){
        ...
        $.ajax({
    		url:'<?php echo base_url().$this->uri->segment(1);?>/reportcommcomment',
    		method: 'POST',
    ...
    function topcommloadmore(storyid, commentid){
        ...
        $.ajax({
            type:'POST',
            url:"<?php echo base_url().$this->uri->segment(1);?>/commloadcomments/topstory",
            data:{'storyid':storyid, 'commentid':commentid},
            success:function(html){
    ...
    function topcommloadsubcomment(storyid, comment_id, subcommentid ){
        ...
        $.ajax({
            type:'POST',
            url:"<?php echo base_url().$this->uri->segment(1);?>/commloadsubcomments/topstory",
            data:{'storyid':storyid, 'comment_id':comment_id,'subcommentid':subcommentid },
            success:function(html){
...
<script type="text/javascript">
    function likestory(id){ // like Story
    	...
    	$.ajax({
    		url:'<?php echo base_url().$this->uri->segment(1);?>/comm_likes',
    		method: 'POST',
    		data: {'story_id':id},
    		dataType:'json',
    		success:function(data){
    ...
    function deletecomm_post(comm_story_id, posted_by){
        ...
        $('#confirmdelpopup').modal().one('click', '#delconfirmed', function (e) {
        	$.ajax({
        		url:'<?php echo base_url().$this->uri->segment(1);?>/deletecomm_post',
        		method: 'POST',
    ...
    function editcomm_post(comm_story_id, posted_by){
    	...
    	$.ajax({
    		url:'<?php echo base_url().$this->uri->segment(1);?>/editcomm_post',
    		method: 'POST',
    		data: {'comm_story_id':comm_story_id, 'posted_by':posted_by},
    		success:function(data){
    		    if(data) {
    ...
    function communitymembers(commuid){
        $.ajax({
            url: '<?php echo base_url().$this->uri->segment(1);?>/communitymembers/'+commuid,
            method: "POST",
            success:function(data){
...
<script>
    $(document).ready(function(){
        ...
        function load_country_data(limit, start) {
            ...
            var communitie_about = $.trim($('h4#communitie_about').text());
            $.ajax({
                url:'<?php echo base_url().$this->uri->segment(1);?>/loadcommustories/'+commuid,
                method:"POST",
                data:{limit:limit, start:start, 'communitie_about':communitie_about},
                cache:false,
                success:function(data){
