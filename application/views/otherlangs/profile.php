            ...
            <div class="" style="display:flex; flex-wrap:wrap; text-justify:center;">
        		...
        		<div class="main-container1">
                    ...
                    <div class="tab-content" id="tabreadingviewall">
                                    ...
                                    <div id="StoryContv" class="StoryCont1" style="text-align:left;">
    			                        <div id="story-sliderv" class="story-slider series">
        			                        <?php $i = 0; foreach($writerseries->result() as $wseriesrow){ if($i < 4){ ?>
        			                    		...
        			                    		<a href="<?php echo base_url($this->uri->segment(1).'/series/'.preg_replace('/\s+/', '-', $wseriesrow->title).'-'.$wseriesrow->sid.'/'.preg_replace('/\s+/', '-', $wseriesrow->title).'-'.$wseriesrow->story_id);?>" class="imagess-style">
                		...
                		<!-- START TAB 2 -->
                		<div class="tab-pane" id="tab_reading" <?php if($readingliststatus != 'public'){ ?>style="display:none;"<?php } ?> style="padding-top:35px;">
            ...
            $.ajax({
                ...
                success: function(result) {
                    if(result.response){
                        ...
                        '<div class="" style="float:left;><a href="<?php echo base_url().$this->uri->segment(1);?>/'+result.response[0].profile_name+'">'+
...
<script>
    ...
    function profilefollowing(userid){
        $.ajax({
    		url:'<?php echo base_url().$this->uri->segment(1);?>/profilefollowing',
    		data:{'userid' : userid },
    		method: 'POST',
    		success:function(data){
    		    $('#tab_2').html(data);
...
/* reading list tab view more */
    function readall(userid, type){
        ...
        $.ajax({
    		url:'<?php echo base_url().$this->uri->segment(1);?>/profilereadall',
