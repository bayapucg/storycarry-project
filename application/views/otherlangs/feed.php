			...
			<div class="" style="margin-top:10px; display:flex;justify-content: center;">
				...
				<div class="hidden-xs side-feed">
					<div class="sidebarvj" style="box-shadow:0 1px 1px 0 rgba(0,0,0,0.14), 0 2px 1px -1px rgba(0,0,0,0.12), 0 1px 3px 0 rgba(0,0,0,0.2)!important">
						<div class="header">
							<p><b>JOINED COMMUNITIES <span class="pull-right"><a href="<?php echo base_url($this->uri->segment(1).'/communities');?>">More</a></span>
							</b></p>
						</div><br>
...
<script type="text/javascript">
    function tab_yourfeed(){ // your feed tab
        $.ajax({
    		url:'<?php echo base_url().$this->uri->segment(1);?>/tab_yourfeed',
    		method: 'POST',
    ...
    function tab_stories(){ // stories or your network tab
        $.ajax({
    		url:'<?php echo base_url().$this->uri->segment(1);?>/tab_stories',
    		method: 'POST',
    		success:function(data){
...
/* top posts */
    ...
    function storyfeed(sid){ //Comments post submit
    	...
    	$.post("<?php echo base_url().$this->uri->segment(1);?>/fscomment",{'comment':comment,'storyid':sid},function(resultdata, status) {
    		var result=JSON.parse(resultdata);
    		if(result.status == -1){
    ...
    function fsloadmore(storyid, commentid){
        ...
        $.ajax({
            type:'POST',
            url:"<?php echo base_url().$this->uri->segment(1);?>/fsloadcomments",
            data:{'storyid':storyid, 'commentid':commentid},
            success:function(html){
    ...
    function fsloadsubcomment(storyid, comment_id, subcommentid ){
        ...
        $.ajax({
            type:'POST',
            url:"<?php echo base_url().$this->uri->segment(1);?>/fsloadsubcomment",
            data:{'storyid':storyid, 'comment_id':comment_id,'subcommentid':subcommentid },
            success:function(html){
...
<script> 
    $(document).ready(function(){ // feed auto scroll
        ...
        function load_country_data(limit, start) {
            $.ajax({
                url:'<?php echo base_url().$this->uri->segment(1);?>/loadmorefeed',
                method:"POST",
...
<script> 
    $(document).ready(function(){ // yourfeed auto scroll
        ...
        function yload_country_data(ylimit, ystart) {
            $.ajax({
                url:'<?php echo base_url().$this->uri->segment(1);?>/loadmoreyourfeed',
                method:"POST",
...
<script> 
    $(document).ready(function(){ // Stories feed auto scroll
        ...
        function sload_country_data(slimit, sstart) {
            $.ajax({
                url:'<?php echo base_url().$this->uri->segment(1);?>/loadmorestoryfeed',
                method:"POST",
