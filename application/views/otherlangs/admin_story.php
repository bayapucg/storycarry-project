...
<?php if(isset($admin_story_view) && ($admin_story_view->num_rows() == 1)){ foreach($admin_story_view->result() as $row) {
    if(!isset($this->session->userdata['logged_in']['user_id']) || ($row->user_id != $this->session->userdata['logged_in']['user_id'])){
        header('Location: '.base_url().$this->uri->segment(1).'/story/'.preg_replace("~[^\p{M}\w]+~u", '-', $row->title).'-'.$row->sid);
    }
?>
    	...
    	<ul class="nav navbar-nav pull-right" style="display: inline-flex;">
			<li class="dropdown write-d">
                ...
                <ul class="dropdown-menu pull-right">
                    <li><a href="<?php echo base_url().$this->uri->segment(1);?>/admin_story_view/<?php echo $row->sid;?>"><i class="fa fa-edit pr-10"></i>STORY EDIT </a></li>
                    <li><a href="<?php echo base_url().$this->uri->segment(1);?>/story_info/<?php echo $row->sid;?>"><i class="fa fa-edit pr-10"></i>INFO EDIT </a></li>
               </ul>
            </li>
...
<div class="tops" style="justify-content:center;">
    <div  style="margin:0 auto;">
	    <div class="main-container">
			...
			<?php $storyid = 0;foreach($admin_story_view->result() as $row) { $storyid = $row->sid; ?>
								...
								<div class="row pt-0">
								    <center>
										<div class="col-sm-12">
        									...
        									<?php } else { ?>
        									    <a href="<?php echo base_url().$this->uri->segment(1).'/'.$row->profile_name; ?>" style="color:#000"><h4><b><?php echo $row->name; ?></b></h4></a>
        									<?php } ?>
        								</div>
									</center>
								...
								<div class="row pt-0">
								    <div style="padding-top:10px;">
    									<span class="" style="float:left">
    										<a href="<?php echo base_url().$this->uri->segment(1).'/'.$row->profile_name;?>" style="color:#000"><h4 style="margin-top:10px"><b> <?php echo $row->name; ?></b></h4></a>
    									</span>
					            ...
					            <div class="card">
			                    	...
			                    	<div>
			                    		<font class="max-lines">
			                    			<a href="<?php echo base_url($this->uri->segment(1).'/story/'.preg_replace("~[^\p{M}\w]+~u", '-', $recentstory->title).'-'.$recentstory->sid);?>">
                                                ...
                                                <div class="comment-text">
                                                    <span class="username" style="padding-top:6px;">&nbsp;<b>    
                                                        <a href="<?php echo base_url().$this->uri->segment(1).'/'.$comment->profile_name; ?>">
                                                            <p class="namers"><?php echo ucfirst($comment->name); ?></p></a></b>/a></b>
