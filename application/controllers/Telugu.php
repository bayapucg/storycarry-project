...
class Telugu extends CI_Controller {
            ...
            if($allusers->num_rows() > 0){ foreach($allusers->result() as $res){
                if($res->user_id != $this->session->userdata['logged_in']['user_id']){
                    ...
                    if(isset($res->profile_image) && !empty($res->profile_image)){
                        $datares[$i]['text'] = '<div style="flex"><img style="height:30px;width:30px; float:left;" src="'.base_url().'assets/images/'.$res->profile_image.'"><span style="padding-left:8px;">'.$res->name.'</span></div>';
                    }else{
                        $datares[$i]['text'] = '<div style="flex"><img style="height:30px;width:30px; float:left;" src="'.base_url().'assets/images/2.png"><span style="padding-left:8px;">'.$res->name.'</span></div>';
                    }
                    $i++;
                }
            }   }
