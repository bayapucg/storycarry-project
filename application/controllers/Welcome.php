...
class Welcome extends CI_Controller {
	...
	public function index() {
		/*if(isset($this->session->userdata['language']) && !empty($this->session->userdata['language'])){
		    $langfullname = $this->User_model->langfullname($this->session->userdata['language']);
		    redirect(base_url().$langfullname.'/home');
		}*/
	...
	public function english(){ //english($language = false){
	    if(isset($language) && !empty($language)) {
	        $langfullname = $this->User_model->langfullname($language);
	        if(isset($langfullname) && !empty($langfullname)){
	            redirect(base_url().$langfullname);
	        }else{
	            redirect(base_url().'english');
	        }
	    }
	...
	public function topwriterloading(){
	    if(isset($_POST['limit'], $_POST['start'])){
	        ...
	        $data['follow_count'] = $this->User_model->follow_count();
		    $data['following'] = $this->User_model->following();
	        $this->load->view('writersloadmore.php',$data);
	    }
	}
	    ...
	    if(!isset($this->session->userdata['logged_in']['user_id']) ){
    		...
    		if ($this->form_validation->run() == false) {
                ...
                $data = array(
                    ...
                    'date' => date("Y-m-d"),
                    'type' => 'series',
                    //'draft' => 'draft', // submits info page add to drafts inititally
                );
	...
	public function addstory_intro($lid){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if($add == 'yes'){
			    redirect(base_url('episode/'.$title.'-'.$lid.'/'.$title.'-'.$lid));
			}else{
		       redirect(base_url('admin-series/'.$title.'-'.$lid.'/'.$title.'-'.$lid));
			}
		}
	}
    ...
    public function addepisode($seriesnamesid, $seriesnamestoryid){
    	...
    	$copyrights = $this->input->post('copyrights');
    	$language = $this->input->post('language');
    	$type = $this->input->post('type');   // default series for new episode
    	...
    	if(isset($response) && ($response->num_rows() > 0)){
    	    ...
    	    redirect(base_url().'admin-series/'.$seriesnamesid.'/'.$seriesnamestoryid);
    	}else{
    ...
    public function story_info($sid){
		...
		if(isset($header['story_info']) && $header['story_info']->num_rows() == 1){
		    ...
		    if(isset($result[0]->type) && ($result[0]->type == 'series') && ($result[0]->sid == $result[0]->story_id)){
		        ...
		        redirect(base_url().'series_edit/'.$sid);
	...
	public function updateseries_info($sid){
        ...
        if(isset($_POST['editstorytype']) && ($_POST['editstorytype'] == 'seriespreface')){
            ...
            redirect(base_url('story_info/'.$sid));
        }
	}
	...
	/*public function story_info_uplode($sid){
		...
		if ($this->form_validation->run() == FALSE) {
		    ...
		    redirect(base_url().'story_info/'.$_POST['hidden']);
		}
...
/*	public function updatestory12($id)
        ...
        if(in_array($file_type, [ 'jpeg', 'png', 'gif', 'jpg' ])) {
        	...
        	//echo base_url().$imagepath;
        	 ...
        	 if(isset($width) && ($width > 700)){
                ...
                /*$newImage = $manipulator->resample(700, null, function ($constraint) {
	...
	public function nano_story() {
		if($this->session->userdata('logged_in')==NULL) redirect(base_url());
		$header['languages'] = $this->User_model->language();
		$this->load->view('nano_story.php',$header);
	}
    ...
    public function communitymembers($commuid){
        ...
        $data['following'] = $this->User_model->following();
        $data['commuid'] = $commuid;
        $this->load->view('communitymembers.php',$data);
    }
    public function memloadcommu($commuid){
        if(isset($_POST['limit'], $_POST['start']) && !empty($_POST['limit'])){
            ...
            $data['following'] = $this->User_model->following();
            $data['commuid'] = $commuid;
            $this->load->view('loadmorecommumem.php',$data);
        }
    }
    ...
    public function community_tpost(){
        if(isset($_POST['comm_id'], $_POST['communitie_about']) && !empty($_POST['comm_id']) && !empty($_POST['communitie_about'])){
            ...
            //$data['communities_story_weekback'] = $this->User_model->communities_story_weekback($_POST['comm_id'],0,6);
            //$data['communities_zerolikes']=$this->User_model->communities_story_zerolikes($_POST['comm_id'],0, 6);
        ...
        if(isset($commid) && !empty($commid)){
            ...
            if(isset($data['communities_story_get']) && ($data['communities_story_get']->num_rows() > 0 )){
                $metatags = $data['communities_story_get']->result();
                    $header['ogtitle'] = $metatags[0]->title;
                    $header['ogdescription'] = strip_tags($metatags[0]->story);
                    $header['ogurl'] = base_url().'community-story/'.$commname.'/'.$comm_storyid;
                if(isset($metatags[0]->image) && !empty($metatags[0]->image)){
                    $header['ogimage'] = $metatags[0]->image;
                }else{
            ...
            redirect(base_url().'community/'.$commname);
        }
	}
    public function loadcommustories($commuid){
 	...
 	public function weektoploadcommustories($commuid){
 	    if(isset($_POST['limit'], $_POST['communitie_about']) && !empty($_POST['start']) && !empty($_POST['communitie_about'])){
 	        $data['toploadcommustories']=$this->User_model->communities_story_weekback($commuid,$_POST['start'], $_POST['limit']);
 	        $data['comm_comment_count'] = $this->User_model->comm_comment_count($commuid);
 	        $data['commuid'] = $commuid;
 	        $data['communitie_about'] = $_POST['communitie_about'];
 	        $this->load->view('loadcommustories.php',$data);
 	...
 	public function zerolikeloadcommustories($commuid){
 	    if(isset($_POST['limit'], $_POST['communitie_about']) && !empty($_POST['start']) && !empty($_POST['communitie_about'])){
 	        $data['toploadcommustories']=$this->User_model->communities_story_zerolikes($commuid,$_POST['start'], $_POST['limit']);
 	        $data['comm_comment_count'] = $this->User_model->comm_comment_count($commuid);
 	        $data['commuid'] = $commuid;
 	        $data['communitie_about'] = $_POST['communitie_about'];
 	        $this->load->view('loadcommustories.php',$data);
	...
	public function seriessubscribe(){
	    if(isset($this->session->userdata['logged_in']['user_id']) && !empty($this->session->userdata['logged_in']['user_id'])){
            $userid = $this->session->userdata['logged_in']['user_id'];
    	    $data['scseries'] = $this->User_model->libraryseries($userid, 'seriessubscribe', 0,8);
            //$data['scstories'] = $this->User_model->librarystories($userid, 'seriessubscribe');
            //$data['sclife'] = $this->User_model->librarylife($userid, 'seriessubscribe');
            //$data['reviews21'] = $this->User_model->reviews2();
            //$data['allusers'] = $this->User_model->allusers();
	...
	public function libseriesloadmore(){
	    if(isset($_POST["limit"], $_POST["start"]) && !empty($_POST['limit'])){
            ...
            $this->load->view('loadmorelibsubscribe.php',$data);
        }
	}
    ...
    public function viewallloadyournetwork(){
        if(isset($this->session->userdata['logged_in']['user_id']) && !empty($this->session->userdata['logged_in']['user_id']) 
         && isset($_POST["limit"]) && isset($_POST["start"])){
            ...
            $this->load->view('viewallloadyournetwork.php',$data);
        }
    }
    
    public function viewallhome($type){
        if(($this->uri->segment(1) == 'series') && ($this->uri->segment(2) == 'latest')) {
            $type = 'series';
        }else if(($this->uri->segment(1) == 'stories') && ($this->uri->segment(2) == 'latest')) {
            $type = 'story';
        }else if(($this->uri->segment(1) == 'lifeevents') && ($this->uri->segment(2) == 'latest')) {
            $type = 'life';
        }else if(($this->uri->segment(1) == 'shortstories')) {
            $type = 'nano';
        }
        $header['gener'] = $this->User_model->gener();
		$header['languages'] = $this->User_model->language();
	...
	public function topviewallhome($type){
	    if(($this->uri->segment(1) == 'series') && ($this->uri->segment(2) == 'top')) {
            $type = 'series';
        }else if(($this->uri->segment(1) == 'stories') && ($this->uri->segment(2) == 'top')) {
            $type = 'story';
        }else if(($this->uri->segment(1) == 'lifeevents') && ($this->uri->segment(2) == 'top')) {
            $type = 'life';
        }else if(($this->uri->segment(1) == 'shortstories')){
            $type = 'nano';
        }
    ...
    public function profile($profilename){
        ...
        if(isset($profilename) && !empty($profilename)){
            ...
            if(isset($this->session->userdata['logged_in']['user_id']) && ($this->session->userdata['logged_in']['user_id'] == $userid)){
                ...
                //$data['clientprofile'] = $this->User_model->client_profile($userid); // new user query changed in writer_profile($userid);
    ...
    public function profilefollowing(){
        if(isset($_POST['userid']) && !empty($_POST['userid'])){
            ...
            $data['following'] = $this->User_model->following();
            $this->load->view('following_names.php',$data);
        }
    }
    ...
    public function profilereadall(){
        if(isset($_POST['id']) && !empty($_POST['id']) && isset($_POST['type']) && ($_POST['type'] == 'series')){
            ...
            $this->load->view('profilereadall.php',$data);
    ...
    public function profileviewall() {
        $type_pid = explode('-',$this->uri->segment(2));
    ...
    /*public function updateprofile($id) {
		...
		$dbemail = $this->User_model->dbemail();
		$user_activation  = 1;
		if(isset($dbemail, $_POST['email']) && !empty($dbemail) && ($_POST['email'] != $dbemail)){
		    $user_activation = 2;
		    $this->User_model->sendEmail($_POST['email']);
		}
		if ($this->form_validation->run() == FALSE) {
    ...
    public function loadmoredrafts(){
        if(isset($_POST['start'], $_POST['limit']) && !empty($_POST['start']) && !empty($_POST['limit'])) {
            ...
            $this->load->view('loadmoredrafts', $data);
        }
    }
    ...
    public function image_crop(){
        if(isset($_POST['img']) && !empty($_POST['img'])){
            ...
            echo base_url().'assets/images/'.$path; */
            $link_arrayfullimg = explode('/',$_POST['fullimg']);
            $imagenamefullimg = end($link_arrayfullimg);
            $config['image_library'] = 'gd2';
	...
	public function communitynotis(){
	    if(isset($this->session->userdata['logged_in']['user_id']) && !empty($this->session->userdata['logged_in']['user_id'])){
	        ...
	        $this->load->view('notificationscommu.php',$data);
	    }
	}
	public function suggestnotis(){
	    if(isset($this->session->userdata['logged_in']['user_id']) && !empty($this->session->userdata['logged_in']['user_id'])){
	        $data['notifications'] = $this->User_model->suggestnotis(0,15);
	        $this->load->view('notificationssugg.php',$data);
	    }
	}
	...
	public function search(){
        ...
        if(isset($_GET['searchtext']) && !empty($_GET['searchtext'])){
    		...
    		/*$data['get_writer'] = $this->User_model->get_top_writer($header['searchtext'], 0,5);
	...
	public function lifeeventtags(){
        ...
        $this->load->view('header.php', $header);
        $this->load->view('lifeeventtags.php', $data);
        $this->load->view('footer.php');
	}
	public function lifeeventtagsloadmore(){
	    if(isset($_POST['start'], $_POST['limit']) && !empty($_POST['start'])){
    	    ...
    	    $this->load->view('lifeeventtagsloadmore.php', $data);
	    }
	}
    ...
    public function blogloadmore(){
        if(isset($_POST['start'], $_POST['limit']) && !empty($_POST['limit'])) {
            ...
            $this->load->view('blogloadmore.php', $data);
        }
    }
    ...
    public function blogloadcomments(){
	     if(isset($_POST["limit"], $_POST["start"], $_POST['blog_id']) && !empty($_POST['blog_id']) && !empty($_POST["limit"])){
            ...
            $this->load->view('blogloadcomments.php',$data);
        }
	}
...
$data['to'] = "318767071276";
 print_r(json_encode($data));
$ch = curl_init();

curl_setopt($ch, CURLOPT_POST, 1);
$headers = array();
$headers[] = "Authorization: key = AAAAZf5q1Dw:APA91bE8iIa337tVA2uUcLDkPj6YHLP-CA_yNqNrcWHTcRLR6He-SqxSfcCM-wSNQJXRoKRyQZ454G2fbPGnSO6vyZ62NrxOusaBsx9OKHoims8LuKWMY_2vdwD8JfygzXgzekg6rUSB";
...
$this->load->view('notifybrowsertest.php');
}
	
	public function testb(){
	    $headers = 'From: info@storycarry.com' . "\r\n" .
	        'Reply-To: info@storycarry.com' . "\r\n" .
	        'Content-type: text/html'."\r\n".
	        'X-Mailer: PHP/' . phpversion();
        $status = mail('saimahi1989@gmail.com','test subject ','test message',$headers);
        echo $status; 
